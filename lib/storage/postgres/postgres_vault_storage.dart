...
final class PostgresVaultStorage implements VaultStorage {
  ...
  /// Доступ к пулу соединений для создания репозиториев.
  Pool get pool => _pool;
  ...
  Future<void> _setTenantContext(Session session) async {
    // SET не поддерживает параметры - используем строковую интерполяцию
    // Экранируем одинарные кавычки для безопасности
    final escapedTenantId = tenantId.replaceAll("'", "''");
    // Используем SET вместо SET LOCAL, так как Pool.run не всегда создаёт транзакцию
  ...
  Future<void> put(String collection, String id, Map<String, dynamic> data) async {
    ...
    // RLS автоматически фильтрует по tenant_id через current_setting
    await _pool.run((Session session) async {
  ...
  Future<Map<String, dynamic>?> get(String collection, String id) async {
    return await _pool.run((Session session) async {
      await _setTenantContext(session);
  ...
  Future<PageResult<Map<String, dynamic>>> queryPage(
    ...
    return await _pool.run((Session session) async {
      ...
      final sql = _buildQuerySql(collection, query);
      final result = await session.execute(sql, parameters: params);
      final data = result.map((row) => row[0] as Map<String, dynamic>).toList();
  ...
  Future<T> transaction<T>(Future<T> Function(VaultStorage tx) action) async {
    // Используем Pool.run для автоматического управления транзакцией
    return await _pool.run((Session session) async {
      // Устанавливаем tenant контекст в начале транзакции
      await _setTenantContext(session);

      // Создаём временный storage который использует session
  ...
  Future<void> clear(String collection) async {
    await _pool.run((Session session) async {
      ...
      await session.execute('DELETE FROM $collection');
    });
  }
  ...
  String _buildCountSql(String collection, VaultQuery query) {
    // RLS автоматически добавляет фильтр по tenant_id
    final sql = StringBuffer('SELECT COUNT(*) FROM $collection WHERE 1=1');
  ...
  List<dynamic> _buildQueryParams(VaultQuery query) {
    // tenant_id больше не нужен - RLS добавляет его автоматически
...
final class _PostgresVaultStorageTransaction implements VaultStorage {
  ...
  @override
  Future<void> put(String collection, String id, Map<String, dynamic> data) async {
    // Контекст tenant уже установлен в родительской транзакции
    await session.execute('''
      INSERT INTO $collection (id, tenant_id, data, created_at, updated_at)
      VALUES (\$1, \$2, \$3, NOW(), NOW())
      ON CONFLICT (id, tenant_id)
